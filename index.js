...
function buildFilter(field, values) { if (!values?.length) return null; if (values.length === 1) return ${field} = ${values[0]}; return ${field} = (${values.join(',')}); }

app.get('/games/ask', async (req, res) => { try { const pergunta = req.query.question || ""; const { title, genreIds, platformIds, themeIds, keywordIds, year, limit } = parseGameQuery(pergunta);
...
} catch (error) { console.error("[IGDB ERROR]", error?.response?.data || error.message); res.status(500).json({ fallback: true, results: [], message: 'Erro na conexão com a IGDB.' }); } });
